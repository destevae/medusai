...
def robomove():
    ...
    while True:
        ...
        tomove = []
        for xarm in xarms:
            tomove.append(xarm.snakebeathalf(amps, speeds[0], phases))
        # end = float(traj[-1])
        # IP = 1
        ...
        tomove = []
        for xarm in xarms:
            tomove.append(xarm.snakebeathalf(amps, speeds[0],[x+1 for x in phases]))
        # end = float(traj[-1])
        # IP = 1
        for i in range(len(tomove[0])):
            start = time.time()
...
def manual_pluck_trigger():
    ...
    while True:
        ...
        if message == b'pluck1' and canStrum1:
            ...
            canStrum1 = False
            pluck_queue.put("pluck1")
        elif message == b'pluck2' and canStrum2:
            print("can no longer strum 2")
            canStrum2 = False
            pluck_queue.put("pluck2")
