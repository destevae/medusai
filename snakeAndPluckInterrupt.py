...
# Placeholder for the other files' threads (let's say they're imported from other files)
# from file1 import optical thread
# from file2 import sideways move thread

# global flag to stop the robomove threads after 60 seconds
stop_robomove = False
robomove_active = False
...
def manual_pluck_trigger():
    global canStrum1
    global canStrum2
    global stop_robomove, robomove_active

    while not stop_robomove:
...
# first 60 seconds, run the snakeAndPluckHit
def start_robomove_threads():
    global stop_robomove
    threading.Thread(target=robomove, daemon=True).start()
    threading.Thread(target=robomove2, daemon=True).start()

# after 60 seconds, switch to amit vision threads
def switch_to_other_threads():
    print("Switching to other threads after 60 seconds.")

    # threading.Thread(target=other_thread1, daemon=True).start()
    # threading.Thread(target=other_thread2, daemon=True).start()
...
def main():
    ...
    # start robomove and manual pluck threads initially
    start_robomove_threads()

    threading.Thread(target=manual_pluck_trigger, daemon=True).start()

    time.sleep(60)

    # after 60 seconds, stop the robomove threads
    global stop_robomove, robomove_active
    stop_robomove = True
    robomove_active = False
    print("snakeAndPluckHit threads stopped after 60 seconds.")

    switch_to_other_threads()

    # Continue running manual pluck trigger
