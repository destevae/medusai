...
right_wrist_history_un = defaultdict(lambda: [])
        ...
        try:
            ...
            for box, kps, track_id in zip(boxes, keypoints, track_ids):
                ...
                if (nose_un[0] >= 648):
                    ...
                    right_wrist_un = right_wrist_history_un[track_id]
                    ...
                    right_wrist_un.append(right_wrist_curr)
                    ...
                    send_rw_un = np.mean(np.array(right_wrist_un), axis=0)
                    ...
                    if len(left_wrist_un) >= 3:
                        left_wrist_un.pop(0)
                    if len(right_wrist_un) >= 3:
                        right_wrist_un.pop(0)
                    if len(left_hip_un) >= 3:
                        left_hip_un.pop(0)
                    if len(right_hip_un) >= 3:
                        right_hip_un.pop(0)

                    send_msg = str(send_coordinate_un) + "/" + str(send_lw_un) + "/" + str(send_lh_un) + "/" + str(send_rh_un) + "/" + str(send_rw_un)
                    ...
                    right_wrist_history_un[track_id] = right_wrist_un
                    print("RIGHTWRIST", right_wrist_un)
            ...
            print("ENCODED", str.encode(str(send_arr)))
